...
int main(int argc, char** argv) {
  if (argc != 9) {
    std::cout << argv[0]
              << " data_file query_file nsg_path search_L search_K hub_path embadding_path result_path"
              << std::endl;
    exit(-1);
  }
  ...
  hnswlib::HierarchicalNSW<float>* cos_index = nullptr;
  ...
  if (embadding_path != "nullptr") {
    ...
    cos_index = new hnswlib::HierarchicalNSW<float>(&space, embadding_num, 256, 256);
    
    for (int i = 0; i < embadding_num; i++) {
      cos_index->addPoint(embadding_data + i * dim, i);
    }
  }
